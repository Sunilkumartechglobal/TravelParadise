...
host: { name: "Rajesh Kumar", image: "", verified: true, responseTime: "Within 1 hour" }
};
...
const boat = HOUSEBOAT_DATA[id] ? { ...DEFAULT_DETAILS, ...HOUSEBOAT_DATA[id] } : DEFAULT_DETAILS;
...
<img src={boat.images[currentImage]} alt="Houseboat" className="houseboat-gallery-image" />
<button onClick={() => setCurrentImage((prev) => (prev + 1) % boat.images.length)} className="houseboat-gallery-btn houseboat-gallery-btn-right"><ChevronRight /></button>
</div>
<div className="houseboat-card">
<h1 className="houseboat-card-title">{boat.name}</h1>
<div className="houseboat-location-row"><MapPin className="w-5 h-5" /><span>{boat.location}</span></div>
</div>
</div>
<div className="houseboat-booking-card-wrapper">
<div className="houseboat-booking-card">
<div className="houseboat-price-display"><span>₹{boat.price.toLocaleString('en-IN')}</span><span>/ night</span></div>
<input type="date" value={checkIn} onChange={e => setCheckIn(e.target.value)} className="houseboat-form-input" />
<input type="date" value={checkOut} onChange={e => setCheckOut(e.target.value)} className="houseboat-form-input" />
<button onClick={handleBooking} disabled={!checkIn || !checkOut} className="houseboat-book-btn houseboat-book-btn-active"><MessageCircle /> Book on WhatsApp</button>
</div>
</div>
</div>
</div>
